...
class CMSNT
{
    ...
    {
        $this->connect();
        $row = $this->ketnoi->query("SELECT * FROM `loaithe` WHERE `type` = '$data' ")->fetch_array();
        return $row['ck'];
    }
    function type_muathe($data)
    {
        $this->connect();
        $row = $this->ketnoi->query("SELECT * FROM `type_muathe` WHERE `type` = '$data' ")->fetch_array();
        return $row['name'];
    }
    function query($sql)
    {
